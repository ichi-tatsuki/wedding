...
function clickJackie(answer) {
    var jackie = document.getElementById(answer);
    var currentRotation = getRotation(jackie);

    jackie.style.opacity = 0; // フェードアウト
    jackie.style.transform = 'rotate(' + currentRotation + 'deg) scale(1.5)'; // 画像を拡大

    setTimeout(() => {
        jackie.style.display = 'none'; // 画像を非表示にする
        //   alert('Thank you')
    }, 500); // アニメーションが完了するまでの時間 (ミリ秒)
    writeToSpreadsheet(answer);
}
